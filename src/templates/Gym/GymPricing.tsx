        ...
        <section id="pricing" className="section-padding bg-gray-900 text-white">
            <div className="container-custom">
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {plans.map((plan) => (
                        <div key={plan.id} className={`relative rounded-2xl p-8 ${plan.highlighted ? 'bg-gradient-to-b border-2' : 'bg-gray-800 border border-gray-700'}`}
                            ...
                            <h3 className="text-2xl font-bold mb-2"><EditableText elementId={plan.nameId} defaultText={plan.nameDefault} tag="span" /></h3>
                            <div className="mb-6">
                                <span className="text-4xl font-bold text-white"><EditableText elementId={plan.priceId} defaultText={plan.priceDefault} tag="span" /></span>
                                <span className="text-gray-400"><EditableText elementId={plan.periodId} defaultText={plan.periodDefault} tag="span" /></span>
                            </div>
                            <ul className="space-y-3 mb-8">
                                {plan.features.map((feat) => (
                                    <li key={feat.id} className="flex items-start">
                                        <Check className="w-5 h-5 mr-2 flex-shrink-0" style={{ color: colors.primary }} />
                                        <span className="text-gray-300"><EditableText elementId={feat.id} defaultText={feat.text} tag="span" /></span>
                                    </li>
                                ))}
                            </ul>
                            <button className="w-full py-3 rounded-lg font-semibold transition-all border"
                                style={plan.highlighted ? { background: colors.primary, color: 'white', borderColor: colors.primary } : { borderColor: colors.primary, color: colors.primary }}>
