...
const LawFirmHeader: React.FC = () => {
    ...
    const navItems = [
        { id: 'lf_nav_home', label: 'Inicio', href: '#home', visible: true },
        { id: 'lf_nav_areas', label: 'Áreas de Práctica', href: '#practice-areas', visible: true },
        { id: 'lf_nav_attorneys', label: 'Nuestro Equipo', href: '#attorneys', visible: true },
        { id: 'lf_nav_contact', label: 'Contacto', href: '#contact', visible: true },
    ];
    ...
    const handleLogoClick = (e: React.MouseEvent) => {
        if (config.isEditing) {
            e.preventDefault();
            const logoElement = document.getElementById('lf_logo');
            if (logoElement) logoElement.click();
        }
    };

    return (
        <header className="sticky top-0 z-50 bg-white/95 dark:bg-stone-950/95 backdrop-blur-md border-b border-stone-200 dark:border-stone-800">
            <div className="container-custom px-6 py-4">
                <div className="flex items-center justify-between">
                    {/* Logo editable */}
                    <a href="/" onClick={handleLogoClick} className="flex items-center space-x-3 group">
                        <div id="lf_logo" data-element-id="lf_logo">
                            <EditableImage
                                elementId="lf_logo"
                                defaultImage=""
                                alt="Kernelize Abogados"
                        ...
                        <div>
                            <EditableText
                                elementId="lf_header_brand_1"
                                defaultText="Kernelize"
                                tag="span"
                                className="text-2xl font-bold text-stone-800 dark:text-stone-200"
                            />
                            <EditableText
                                elementId="lf_header_brand_2"
                                defaultText="Abogados"
                                tag="span"
                                className="text-xl font-semibold text-amber-900 dark:text-amber-200 ml-1"
                    ...
                    <nav className="hidden md:flex items-center space-x-8">
                        {navItems.filter(item => item.visible).map((item) => (
                            <a
                                ...
                                className="text-stone-600 dark:text-stone-300 hover:text-stone-900 dark:hover:text-stone-100 font-medium transition-colors"
                            >
